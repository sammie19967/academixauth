...
export async function verifySMSCode(confirmationResult, code) {
  try {
    ...
    try {
      const userPayload = {
        uid: userToSync.uid,
        email: userToSync.email || `phone-${userToSync.uid}@temp.local`,
      ...
      console.log("[DEBUG] Syncing phone user to MongoDB");
      ...
      const data = await res.json();
      console.log("[DEBUG] User successfully synced to MongoDB");
      
    } catch (e) {
      console.warn("Failed to sync phone user to MongoDB:", e.message);
